...
contract CallRegisterController is AdminControl
{
	...
	{		
		...
		uint256 shared ; // shared percent
		uint scope; // 0: No discount value, 1: discount value
		bool status; // 					
		string title;
		string code;
		address creator;
	...
	mapping(bytes => RefCodeInfo) public colRefCodeInfo;
	
	string[] public RefCodeList;
	
	address private _contractRegister = address(0);

	function addRefCode(string memory code,  string memory title, uint scope, uint256 discount, uint256 shared, address receiver, uint expired) public
	{
	
		string memory _code = StringUtil.toLower(code);
		
		RefCodeInfo memory objRefCodeInfo;
		
		objRefCodeInfo.creator = msg.sender;
		objRefCodeInfo.discount = discount;
		objRefCodeInfo.shared = shared;
		objRefCodeInfo.receiver = receiver;
        objRefCodeInfo.title = title;
        objRefCodeInfo.code = _code;
        objRefCodeInfo.status = true;
		objRefCodeInfo.scope = scope;
		objRefCodeInfo.expired = expired;
		
		colRefCodeInfo[bytes(_code)] = objRefCodeInfo;
		
		RefCodeList.push(_code);
	}
	
	function editRefCode(string memory code, string memory title, uint scope, uint256 discount, uint256 shared, address receiver, uint expired) public
	{
	...
	function callRegisterDomain(address to, string memory domain, string memory tld) external onlyMinterController {
	...
	function callBuyDomain(address to, string memory domain, string memory tld, string memory code, string memory inviter) external payable 
	{
		require(to!= address(0), "To address is null");
